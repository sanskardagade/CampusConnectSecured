...
const StudentLogs = () => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      const response = await fetch(
        `http://69.62.83.14:9000/api/faculty/students-logs?${queryParams}`,
        {
          method: "GET",
          headers: {
  ...
  const toggleAttendanceStatus = async (logIndex, currentStatus) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://69.62.83.14:9000/api/faculty/students-logs/update-attendance', {
        method: 'PUT',
        headers: {
