...
export default function LeaveApplication() {
  ...
  const handleApprove = async (application) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/principal/faculty-leave-approval/${application.ErpStaffId}`,
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000/api/principal/faculty-leave-approval/${selectedApplication.ErpStaffId}`,
