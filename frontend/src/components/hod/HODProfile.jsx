  ...
  useEffect(() => {
    const fetchHODProfile = async () => {
      try {
        ...
        console.log('HOD profile response:', response.data)
      ...
      <div className="pt-16">
        <div className="p-6 max-w-xl mx-auto bg-white rounded-2xl shadow-lg border border-red-100">
          <div className="flex flex-col items-center mb-6">
            ...
            <h1 className="text-3xl font-extrabold text-red-800">HOD Profile</h1>
            <p className="text-gray-500">Welcome back, {hod.name || 'HOD'}!</p>
          </div>

          <div className="space-y-4 px-2">
            <ProfileField label="Name" value={hod.name} />
            <ProfileField label="Faculty ID" value={hod.erpStaffId} />
            <ProfileField label="Department" value={hod.department} />
            <ProfileField label="Email" value={hod.email} />
            {/* <ProfileField label="Start Date" value={hod.startDate} />
            <ProfileField label="End Date" value={hod.endDate || 'Current'} /> */}
            <ProfileField label="Status" value={hod.isActive ? 'Active' : 'Inactive'} />
          </div>
