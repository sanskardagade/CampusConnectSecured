...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const response = await axios.get('http://82.112.238.4:9000/api/admin/dashboard', {
  ...
  const fetchPendingLeaves = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('http://82.112.238.4:9000/api/admin/faculty-leave-approval', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchPresentFacultyStaffSummary = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('http://82.112.238.4:9000/api/admin/present-faculty-staff-summary', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchAllMembers = async () => {
    try {
      ...
      const response = await axios.get(
        'http://82.112.238.4:9000/api/admin/all-members',
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const fetchDepartmentMembers = async (deptId, type) => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await axios.get(
        `http://82.112.238.4:9000/api/admin/members?deptId=${deptId}&type=${type}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setMembers(response.data.members);
  ...
  const fetchMemberProfile = async (memberId) => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await axios.get(
        `http://82.112.238.4:9000/api/admin/profile/${memberId}?type=${selectedType}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      if (selectedType === 'faculty') {
        ...
        try {
          const logsRes = await axios.get(
            `http://82.112.238.4:9000/api/admin/faculty-logs/${memberId}`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
        ...
        try {
          const logsRes = await axios.get(
            `http://82.112.238.4:9000/api/admin/staff-logs/${memberId}`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
  ...
  const handleDownloadAttendance = async (format) => {
    ...
    try {
        const token = localStorage.getItem('token');
        const response = await axios.get('http://82.112.238.4:9000/api/admin/faculty-attendance-report', {
            headers: { Authorization: `Bearer ${token}` },
  ...
  const fetchPresentFacultyList = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('http://82.112.238.4:9000/api/admin/present-faculty-today', {
        headers: { Authorization: `Bearer ${token}` }
      });
      const facultyList = res.data.presentFaculty || [];
      setPresentFacultyList(facultyList);
      setPresentFaculty(facultyList.length); // Always update dashboard count
    } catch (e) {
  ...
  const fetchPresentStaffList = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('http://82.112.238.4:9000/api/admin/present-staff-today', {
        headers: { Authorization: `Bearer ${token}` }
      });
      const staffList = res.data.presentStaff || [];
      setPresentStaffList(staffList);
      setPresentStaff(staffList.length); // Always update dashboard count
    } catch (e) {
      setPresentStaffList([]);
      setPresentStaff(0);
    } finally {
  ...
  const handleNavigateLeaveApproval = () => {
    navigate('/admin/faculty-leave-approval');
  };
  ...
  const fetchFacultyDeptTotals = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('http://82.112.238.4:9000/api/admin/faculty-department-counts', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const fetchStaffDeptTotals = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('http://82.112.238.4:9000/api/admin/staff-department-counts', {
        headers: { Authorization: `Bearer ${token}` }
      });
      // Convert to map: { department_id: count }
      const map = {};
  ...
  const handlePreviewStaffPdf = async (deptId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const today = dayjs().format('YYYY-MM-DD');
      const response = await axios.get('http://82.112.238.4:9000/api/admin/staff-attendance-report', {
        headers: { Authorization: `Bearer ${token}` },
        params: { departmentId: deptId, format: 'pdf', fromDate: today, toDate: today },
        responseType: 'blob',
      });
            ...
            <div className="bg-white rounded-xl shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto relative p-6">
              ...
              <div className="flex justify-center mt-4">
                <button
                  ...
                  onClick={() => { setShowPresentFacultyModal(false); navigate('/admin/faculty-report'); }}
                >
                 Report
                </button>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto relative p-6">
              ...
              <div className="flex justify-center mt-4">
                <button
                  ...
                  onClick={() => { setShowPresentStaffModal(false); navigate('/admin/faculty-report'); }}
                >
                  Download Report
                </button>
              </div>
